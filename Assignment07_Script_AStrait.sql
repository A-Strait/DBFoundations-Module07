...
-- 2020-11-29,AlexStrait,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_StraitAlex')
	 Begin 
	  Alter Database [Assignment07DB_StraitAlex] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_StraitAlex;
	 End
	Create Database Assignment07DB_StraitAlex;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_StraitAlex;
...
As
	Select 
		...
		[InventoryDate] = DateName(mm, InventoryDate) + ', ' +  DateName(yyyy, InventoryDate),
		[InventoryCount],
		[PreviousMonthCount] = IIF(Year(InventoryDate) = 1996, 0, IIF(Lag(ProductName) Over (Order By ProductName, Month(InventoryDate)) = ProductName, IsNull(Lag(InventoryCount) Over (Order By ProductName, Month(InventoryDate)), 0), 0))
